...
/*$vite$:1*/
/* Deliberately raises .sb-arow's max-height above the 44px set in the minified block
   above — the status subtitle line (.sb-astatus) added below the agent name needs the
   extra room, or it clips. Later rule wins at equal specificity; not a leftover. */
...
(function () {
  ...
  /* State word + note, prefixed "was " once the peer is no longer online —
     independent of `recent`: a "Recently offline" row is already filed as
     offline, not mid-fade, so it renders past-tense too. `null` when the
     peer has never reported a status. */
  function statusLine(a) {
    if (!a.status) { return null; }
    var prefix = a.life === "online" ? "" : "was ";
    var note = a.statusNote ? ": " + a.statusNote : "";
    return prefix + a.status + note;
  }
